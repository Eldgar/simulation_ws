...
class TestTortoiseBotWaypoints(unittest.TestCase):
    def setUp(self):
        ...
        self.finished_before_timeout = self.client.wait_for_result(rospy.Duration(40.0))

        if not self.finished_before_timeout:
            rospy.logerr("Action server did not finish within 40 seconds! Marking test as failed.")
        
        # 6) Store result and final pose for both tests to use
        self.result = self.client.get_result()
        rospy.sleep(0.5)  # Ensure final odometry update

        self.final_x = self.current_position.x
        self.final_y = self.current_position.y
        self.desired_yaw = math.atan2(self.goal.position.y, self.goal.position.x)
        self.final_yaw = self.current_yaw

        rospy.loginfo(f"Action server success?: {self.result.success if self.result else 'Unknown'}")
        rospy.loginfo(f"Final position: ({self.final_x:.2f}, {self.final_y:.2f})")
        rospy.loginfo(f"Desired yaw: {self.desired_yaw:.2f} rad, Final yaw: {self.final_yaw:.2f} rad")

    def exit_due_to_timeout(self):
        """Exit immediately if the action server does not connect within 3 seconds."""
        if not self.server_connected:
            rospy.logerr("Failed to connect to action server within 3 seconds. Exiting immediately.")
            sys.exit(1)
    ...
    def test_final_position(self):
        ...
        self.assertTrue(self.result.success, "Action server did not report success!")
    ...
    def test_final_yaw(self):
        ...
        if not self.finished_before_timeout:
            self.fail("Skipping yaw test because action server did not complete in time!")

        yaw_err = abs(self.final_yaw - self.desired_yaw)

        self.assertTrue(self.result.success, "Action server did not report success!")
        self.assertTrue(yaw_err < 0.22,
                        f"Robot's final yaw angle is incorrect (err={yaw_err:.3f} rad)")

if __name__ == '__main__':
    rostest.rosrun('tortoisebot_waypoints', 'test_waypoints', TestTortoiseBotWaypoints)
